...
```bash
dotnet ef database update --connection "Host=prod-host;Database=notesdb;..."
```

**Option B — Apply migrations on startup** (add to `Program.cs`):
```csharp
...
**Option C — Add to CI/CD pipeline** as a pre-deploy step.
